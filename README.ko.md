...
### 5. 다단 레이아웃 지원 (1~3단 컬럼)

- 본문 영역을 **`//column`** 태그로 묶어 다양한 다단 레이아웃을 구성할 수 있습니다.
- **`//column-1`**, **`//column-2`**, **`//column-3`** 마커를 사용하여 최대 3단까지 나란히 정렬되는 컬럼을 작성할 수 있습니다.
- **스마트 자동완성:** 에디터에서 컬럼 마커를 적고 `Enter`를 누르면 짝을 이루는 닫는 마커가 자동으로 생성됩니다.
- **독립적인 페이지 절단 & 분할(Splitting):**
  - 특정 단 내부의 문단이나 리스트가 페이지 한계선을 넘어가면, 해당 단의 **넘치는 문장/아이템만 다음 페이지의 해당 단으로 자연스럽게 잘려서(Reflow)** 넘어가며 다른 단의 레이아웃에는 영향을 주지 않습니다.
  - 번호 리스트(`<ol>`)가 페이지를 넘어 잘려도 이전 번호가 자연스럽게 이어집니다.
- **컬럼 로컬 페이지 나누기:** 단 안에서 **`//page`**를 입력하면 **오직 해당 단의 내용만 다음 페이지의 단으로 강제 분기**되며, 인접한 다른 단은 영향을 받지 않고 그대로 1페이지에 잔류합니다.
![Multi Column Layout](./assets/2026-06-30_multi_layout.png)

### 6. 클릭 한 번으로 PDF 내보내기 (목차 책갈피 포함)
